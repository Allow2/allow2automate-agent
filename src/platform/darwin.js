...
/**
 * macOS platform-specific process management
 */
export default {
  /**
  ...
  async getProcessList() {
    try {
      ...
      const { stdout } = await execPromise('ps -Ao pid,comm,user,%cpu,%mem');
  ...
  async getProcessDetailsByPID(pid) {
    try {
      const { stdout } = await execPromise(`ps -p ${pid} -o pid,comm,user,%cpu,%mem,lstart`);
      const lines = stdout.split('\n');
